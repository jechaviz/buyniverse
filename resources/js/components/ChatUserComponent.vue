<template>
    <div v-if="notify">
        <div v-for="(user, index) in users" :key="index" @click="startChat(user.id)" class="wt-ad" :id="'wt-chat-'+ user.id" :class="[active_id===user.id ? 'wt-active' : '' , user.status_class]">
    ...
    <div v-else>
        <div v-for="(user, index) in users" :key="index" @click="startChat(user.id)" class="wt-ad" :id="'wt-chat-'+ user.id" :class="[active_id===user.id ? 'wt-active' : '']">
            <figure v-if="user.image"><img :src="image_path+user.image" :alt="user.image_name"></figure>
            <div class="wt-adcontent">
                <h3 v-if="user.name">{{user.name}}</h3>
                <span v-if="user.tagline">{{user.tagline}}</span>
...
import Event from '../event.js';
    export default{
        ...
        methods: {
            getstarted() {
                //console.log('asset url '+APP_ASSET_URL);
