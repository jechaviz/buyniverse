						...
						@foreach ($hired_services as $service)
							...
							<tr class="del-{{{ $service['status'] }}}">
								...
								<td data-th="Action">
									<span class="bt-content">
										<div class="">
											<a href="{{{url('employer/service/'.$service['id'].'/'.$service->pivot->id.'/hired')}}}" class="wt-btn" style="background-color: rgb(255, 255, 255) !important; color: rgb(180, 177, 177); font: inherit; border: 1px solid rgb(180, 177, 177); outline: none; border-radius: 0px; padding: 5px 10px;">{{ trans('lang.view') }}</a>
										</div>
									</span>
								</td>
							</tr>	
						...
						@foreach ($completed_services as $service)
							...
							<tr class="del-{{{ $service['status'] }}}">
								...
								<td data-th="Action">
									<span class="bt-content">
										<div class="">
											<a href="{{{url('employer/service/'.$service['id'].'/'.$service->pivot->id.'/completed')}}}" class="wt-btn" style="background-color: rgb(255, 255, 255) !important; color: rgb(180, 177, 177); font: inherit; border: 1px solid rgb(180, 177, 177); outline: none; border-radius: 0px; padding: 5px 10px;">{{ trans('lang.view') }}</a>
										</div>
									</span>
								</td>
							</tr>
					...
					<tbody>
						@foreach ($cancelled_services as $service)
							@php 
								...
								$seller = Helper::getServiceSeller($service['id']);
								$freelancer = App\User::find($seller->user_id);
								$report = Helper::getPivotServiceReport($service->pivot->id);
							@endphp
							<tr class="del-{{{ $service['status'] }}}">
								<td data-th="Service Title">
									<span class="bt-content">
								...
								<td>
									<span class="bt-content">
										<div class="wt-actionbtn">
											<a href="javascript:void(0);" v-on:click.prevent="showReview('{{ $service->pivot->id }}')" class="wt-viewinfo wt-btnhistory wt-reasonbtn">{{ trans('lang.reason') }}</a>
										</div>
									</span>
								</td>
								<td data-th="Action">
									<span class="bt-content">
										<div class="">
											<a href="{{{url('employer/service/'.$service['id'].'/'.$service->pivot->id.'/cancelled')}}}" class="wt-btn" style="background-color: rgb(255, 255, 255) !important; color: rgb(180, 177, 177); font: inherit; border: 1px solid rgb(180, 177, 177); outline: none; border-radius: 0px; padding: 5px 10px;">{{ trans('lang.view') }}</a>
										</div>
									</span>
								</td>
							</tr>
							<b-modal ref="myModalRef-{{ $service->pivot->id }}" class="wt-uploadrating" hide-footer title="{{{trans('lang.rejection_reason')}}}" v-cloak>
								<div class="wt-modalbody modal-body">
									<div class="wt-description">
