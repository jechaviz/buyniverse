								...
								<tbody>
									@foreach ($services as $service)
										...
										<tr class="del-{{{ $service['status'] }}}">
														...
														<fieldset>
															<div class="form-group">
																<span class="wt-select">
																	...
																	<select id="{{ $service['id'] }}-service_status" name="status" data-placeholder="{{ trans('lang.select_status') }}">
																		@foreach ($status_list as $status)
																			<option value="{{ $status->id }}" @if ($status->id == $service['status']) selected @endif>{{ $status->name }}</option>
																		@endforeach
																	</select>
											...
											<td data-th="Action" style="min-width: 130px;">
												<span class="bt-content">
													<div class="">
													<a href="{{{route('edit_service',$service['id'])}}}"><button class="btn">{{ trans('lang.edit')}}</button></a>
													<div class="dropdown">
