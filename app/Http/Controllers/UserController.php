...
class UserController extends Controller
{
    ...
    {
        if (!empty($id)) {
            $package_options = Helper::getPackageOptions(Auth::user()->role);
        ...
        if (!empty($id) && Auth::user()) {
            ...
            if (file_exists(resource_path('views/extend/back-end/package/bank-checkout.blade.php'))) {
                return view::make(
                    ...
                    compact('product_id', 'title', 'symbol', 'mode', 'bank_detail', 'order', 'cost', 'subtitle', 'options', 'type', 'job')
                );
            } else {
                return view::make(
    ...
    public function submitTransection(Request $request)
    {
        ...
        if (!empty($request)) {
            ...
            if($request->total_cost)
                $product_price = $request->total_cost;
            else
    ...
    public function changeOrderStatus(Request $request)
    {
        ...
        if (!empty($request)) {
            if (!empty($request['id']) && !empty($request['status'])) {
                ...
                } elseif ($order->type == 'quiz') {
                    //dd('quiz');
                    $item_type = 'quiz';
                    $quiz = Quiz::find($order->product_id);
                    $title = $quiz->title;
                    $amount = $quiz->price;
                    //new quiz has to be generated as a replica of original
                    $newquiz = new Quiz;
                    $newquiz->title = $quiz->title;
                    $newquiz->price = 0;
                    $newquiz->user_id = $order->user_id;
                    $newquiz->save();
    ...
    {
        if (Auth::user()->getRoleNames()[0] != 'admin' && Auth::user()->role === 'employer') {
            $currency   = SiteManagement::getMetaValue('commision');
            $symbol = !empty($currency) && !empty($currency[0]['currency']) ? Helper::currencyList($currency[0]['currency']) : array();
            $invoices = array();
            $expiry_date = '';
    ...
    public function showOrders()
    {
        ...
        //dd($orders);
        //dd(Helper::getUnserializeData($orders[0]->invoice->transection_doc)[0]);
    ...
    {
        //dd('temp image');
        $path = Helper::assetPath() . '/public/uploads/users/temp/';
