...
class Service extends Model
{
    ...
    public function storeService($request, $image_size = array())
    {
        ...
        if (!empty($request)) {
            ...
            $old_path = Helper::assetPath() . '/uploads/services/temp';
            $new_path = Helper::assetPath() . '/uploads/services/' . $user_id;
     ...
     * @return \Illuminate\Http\Response
     */
    public function storeAPIService($request, $user_id, $image_size = array())
    {
        $json = array();
        if (!empty($request)) {
            $random_number = Helper::generateRandomCode(8);
    ...
    public function updateService($request, $id, $image_size = array())
    {
        ...
        if (!empty($request)) {
            ...
            $old_path = Helper::assetPath() . '/uploads/services/temp';
            $new_path = Helper::assetPath() . '/uploads/services/' . $user_id->user_id;
            $service_attachments = array();
            if (!empty($request['attachments'])) {
                $attachments = $request['attachments'];
                foreach ($attachments as $key => $attachment) {
    ...
    public static function getSearchResult(
        ...
        $json = array();
        $services = Service::select('*')->where('status', 'published');
        $service_id = array();
        $filters = array();
