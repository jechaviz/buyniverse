...
class EmailHelper extends Model
{
    ...
    public static function getEmailHeader()
    {
        ...
        $email_banner = !empty($setting) && !empty($setting[0]['email_banner']) ? url('public/uploads/settings/email/'.$setting[0]['email_banner']): '';
        ?>
    ...
    public static function getEmailFooter()
    {
        ...
        $copyright = !empty($setting) && !empty($setting['copyright']) ? $setting['copyright'] : 'Copyright Buyniverse All Rights Reserved';
        ?>
        </div>
        </div>
        </div>
    ...
    public static function getSiteLogo()
    {
        ...
        if (!empty($email_data[0]['email_logo'])) {
            ...
            return !empty($logo) ? url('public/uploads/settings/email/'.$logo) : url('public/images/logo.png');
        } else {
            $setting = SiteManagement::getMetaValue('settings');
            $logo = !empty($setting) && !empty($setting[0]['logo']) ? $setting[0]['logo'] : '';
            return !empty($logo) ? url('public/uploads/settings/general/'.$logo) : url('public/images/logo.png');
        }
    }
    ...
    public static function getSignature()
    {
        ...
        $sender_tagline = !empty($setting) && !empty($setting[0]['sender_tagline']) ? $setting[0]['sender_tagline'] : 'Worketic A Better Workplace for Employers and Providers';
