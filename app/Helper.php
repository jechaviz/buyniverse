...
use App\Live_contest_participants;
use Carbon\Carbon;
...
class Helper extends Model
{
    ...
    public static function notifycontest() 
    {
        
        $now = Carbon::now();
        $contests = Live_contest::all();
        foreach($contests as $value)
        {
            
            $contestdate = Carbon::parse($value->start_date);
            if(!$contestdate->isPast())
            {
                $diff = $now->diffInDays($contestdate);
                if($diff == 1)
                {
                    $job = Job::find($value->job_id);
                    $participant = Live_contest_participant::where('live_id', $value->id)->get();
                    foreach($participant as $value1)
                    {
                        $user = User::find($value1->user_id);
                        Mail::to($user->email)->send(new Notifycontest($job));
                        Mail::to('sadiqueali786@gmail.com')->send(new Notifycontest($job));
                    }
    ...
    {
        //str_replace("/uploads", "/public/uploads", $temp_path);
        //dd($temp_path, $image);
        $json = array();
        if (!empty($image)) {
            ...
            if ($extension === "jpg" || $extension === "png") {
                ...
                // save original image size
                $img = Image::make($image);
                //dd('in img', $img, $temp_path);
                $img->save($temp_path . '/' . $file_original_name);
                //$img->move($temp_path, $image);
                $json['message'] = trans('lang.img_uploaded');
                $json['type'] = 'success';
                return $json;
            } else {
    ...
    public static function getProfileImage($user_id, $size = '')
    {
        ...
        if($user_id == 22)
        {
            //dd($user_id . '/' . $profile_image.'-'.file_exists(self::publicPath() . '/uploads/users/' . $user_id . '/' . $profile_image));
        }
        
        if (!empty($size)) {
            if (file_exists(self::assetPath() . '/uploads/users/' . $user_id . '/' . $size . $profile_image)) {
                return !empty($profile_image) ? '/uploads/users/' . $user_id . '/' . $size . $profile_image : 'images/user.jpg';
            } else if (file_exists(self::assetPath() . '/uploads/users/' . $user_id . '/' . $profile_image)) {
    ...
    public static function getHomeBanner($type, $size = '')
    {
        ...
        $banner_title  = !empty($banner_settings) && !empty($banner_settings['banner_title']) ? $banner_settings['banner_title'] : 'Hire expert Providers';
    ...
    public static function getFirstHomeSlider()
    {
        ...
        $data['title'] = 'Hire expert Providers';
